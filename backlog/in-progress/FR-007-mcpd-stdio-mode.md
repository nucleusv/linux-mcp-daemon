...
- [ ] Glama Dockerfile config switched to `CMD ["mcpd", "stdio"]` (plus the build steps) and Glama's build passes; listing shows in Glama search (closes the rest of FR-006's Glama item).
...
| T5 | Client: Claude Desktop | `claude_desktop_config.json` `command: mcpd, args: [stdio]` on a Linux box | tools listed and callable | | |
| T6 | Over SSH | client command `ssh host mcpd stdio` | works, no port opened | | |
...
| T9 | Glama | Glama admin → Build with `CMD ["mcpd","stdio"]` | build and checks pass | | |
