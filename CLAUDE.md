...
8. Add/update `docs/website/docs/tools/<group>/<command>.md`. Note: despite ARCHITECTURE.md mentioning `scripts/generate_docs.py`, **that script does not currently exist** — write the page by hand, following an existing page's format (see `docs/website/docs/tools/services/manage.md`); Docusaurus auto-generates the sidebar from the folder structure.
