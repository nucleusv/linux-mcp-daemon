...
ENDPOINT=$(curl -s -N -H "Authorization: Bearer my-secret" http://localhost:9091/sse | grep -m 1 "endpoint" | sed 's/.*endpoint=\([^"]*\).*/\1/')
echo "Got endpoint: $ENDPOINT"

# Fetch Tools
echo -e "\n--- Requesting tools/list ---"
curl -s -X POST "http://localhost:9091$ENDPOINT" \
