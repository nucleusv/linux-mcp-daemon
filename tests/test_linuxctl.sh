...
if echo "$OUTPUT" | grep -q "Successfully connected to mcpd daemon!"; then
    ...
    ./linuxctl -token "$TOKEN" -server "$DAEMON_URL" auth/sudo-rules | grep "list_directory" > /dev/null

    echo "Running 'linuxctl files/list-directory /var/log --privileged true' (positional arg test)..."
    ./linuxctl -token "$TOKEN" -server "$DAEMON_URL" files/list-directory /var/log --privileged true | grep "syslog" > /dev/null || echo "Could not find syslog, but command succeeded"

    echo "Running 'linuxctl disks/disk-usage --path /var/log --privileged true'..."
    ./linuxctl -token "$TOKEN" -server "$DAEMON_URL" disks/disk-usage --path /var/log --privileged true | grep "Total size" > /dev/null

    echo "Running 'linuxctl disks/disk-free --path / --privileged true'..."
    ./linuxctl -token "$TOKEN" -server "$DAEMON_URL" disks/disk-free --path / --privileged true | grep "Filesystem" > /dev/null
