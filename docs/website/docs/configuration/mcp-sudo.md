...
**Symlinks.** The path check is on the path as written, and a symlink under an allowed directory could lead anywhere (`/var/www/x -> /etc/shadow`). So when a root call is limited by `paths:` - any list that doesn't include `/` - the tool doesn't follow symlinks in **any** component of the path: `files/read`, `files/create`, `files/update`, `files/list`, `files/filetype`, `files/find` and `disks/usage` open the path one component at a time with `O_NOFOLLOW` and refuse (`refusing to follow a symbolic link`) instead of escaping. A symlink as the last component is still *reported* by `files/list` and `files/filetype` (`inode/symlink`), just never followed. `files/chmod` and `files/chown` never follow symlinks at all. With `paths: ["/"]` symlinks are followed as usual: the refusal exists so a link can't carry a root call *outside* the allowed paths, and with the whole filesystem allowed there's no outside - wherever a link points is allowed already. Refusing them there would protect nothing and only break ordinary reads: `/etc/resolv.conf`, for one, is a link to `/run/systemd/resolve/stub-resolv.conf` on Ubuntu.
...
| neither | fails: `cannot switch to the host's filesystem - is the mcpd container running with --privileged --pid host? (no host in view - PID 1 is in this process's own mount namespace; if mcpd runs directly on the host, set worker.containerized: false)`; mcpd also warns at startup that privileged calls will fail |

A privileged call never runs as root inside the container by mistake: it runs on the host or fails. Check once after installing that it reports the host: `linuxctl get system os-release --privileged true`.
