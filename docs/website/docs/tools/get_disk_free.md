...
This package implements the `get_disk_space` tool (equivalent to the native Unix `df` command). It uses `syscall.Statfs` to calculate available disk space and inodes instantly without shelling out.
